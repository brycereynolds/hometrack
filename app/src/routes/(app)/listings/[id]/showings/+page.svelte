<script lang="ts">
	...
	const listingShowings = $derived(showings.filter((s) => s.listingId === listing?.id));

	const upcomingShowings = $derived(
		listingShowings.filter((s) => new Date(s.date) >= new Date('2026-04-09'))
	);
	const pastShowings = $derived(
		listingShowings.filter((s) => new Date(s.date) < new Date('2026-04-09'))
	);

	const veryInterested = $derived(listingShowings.filter((s) => s.interestedLevel === 'very').length);
	const somewhatInterested = $derived(listingShowings.filter((s) => s.interestedLevel === 'somewhat').length);
	...
	onMount(() => {
		...
		funnelChart = new Chart(funnelCanvas, {
			...
			data: {
				...
				datasets: [
					{
						data: [listingShowings.length, veryInterested, listing?.offersCount ?? 0],
						backgroundColor: ['#C4704B', '#D4956B', '#5B8BA5'],
						borderWidth: 0,
						borderRadius: 6,
						barPercentage: 0.6
					}
...
{#if listing}
	<div class="space-y-6">
		<div class="flex items-center justify-between">
			<div>
				<h2 class="font-serif text-lg font-semibold">Showings</h2>
				<p class="text-sm text-muted-foreground">{listingShowings.length} total showings</p>
			</div>
			<Button size="sm">
		...
		<div class="grid gap-6 lg:grid-cols-2">
			<Card>
				<CardHeader>
					<CardTitle class="font-serif text-base">Conversion Funnel</CardTitle>
					<CardDescription>Showings to offers pipeline</CardDescription>
				</CardHeader>
				<CardContent>
					<div class="h-48">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Weekly showing activity</CardDescription>
				</CardHeader>
				<CardContent>
					<div class="h-48">
		...
		{#if upcomingShowings.length > 0}
			<Card>
				<CardHeader>
					<CardTitle class="font-serif text-base">Upcoming Showings</CardTitle>
				</CardHeader>
				<CardContent>
					<div class="divide-y">
		...
		{#if pastShowings.length > 0}
			<Card>
				<CardHeader>
					<CardTitle class="font-serif text-base">Past Showings & Feedback</CardTitle>
				</CardHeader>
				<CardContent>
					<div class="divide-y">
						{#each pastShowings as showing}
							{@const interest = getInterestBadge(showing.interestedLevel)}
							...
							<div class="py-4 first:pt-0 last:pb-0">
								<div class="flex items-start justify-between gap-4">
									<div class="flex items-start gap-3">
										<Avatar class="size-9 shrink-0">
											<AvatarFallback class="text-xs bg-muted">
												{showing.agentName.split(' ').map((n) => n[0]).join('')}
											</AvatarFallback>
										</Avatar>
										<div>
											<div class="flex items-center gap-2">
									...
									<Badge variant="outline" class="text-[10px] shrink-0 {interest.color}">
										<InterestIcon class="mr-1 size-3" />
										{interest.label}
									</Badge>
								</div>
								...
								{#if showing.rating}
									<div class="mt-2 ml-12 flex items-center gap-1">
										{#each getStars(showing.rating) as filled}
											<Star class="size-3.5 {filled ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}" />
										{/each}
								...
								{#if showing.feedback}
									<div class="mt-2 ml-12 rounded-md bg-muted/50 p-3">
										<div class="flex items-start gap-2">
											<MessageSquare class="size-3.5 text-muted-foreground mt-0.5 shrink-0" />
											<p class="text-sm text-muted-foreground">{showing.feedback}</p>
										</div>
									</div>
								{/if}
							</div>
		...
		{#if listingShowings.length === 0}
			<Card>
				<CardContent class="flex flex-col items-center justify-center py-12">
					<Calendar class="size-10 text-muted-foreground/30 mb-3" />
					<p class="text-sm text-muted-foreground">No showings scheduled yet.</p>
					<Button variant="outline" size="sm" class="mt-3">Schedule First Showing</Button>
				</CardContent>
			</Card>
		{/if}
	</div>
