...
{#if listing}
	<div class="space-y-6">
		...
		<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
			<Card><CardContent class="p-4"><div class="flex items-center gap-2"><div class="rounded-lg bg-primary/10 p-2"><Eye class="size-4 text-primary" /></div><div><p class="text-xs text-muted-foreground">Total Views</p><p class="text-lg font-bold">{formatNumber(listing.zillowViews ?? 0)}</p></div></div><div class="mt-2 flex items-center gap-1 text-xs text-green-600"><ArrowUpRight class="size-3" /><span>+12% vs last week</span></div></CardContent></Card>
